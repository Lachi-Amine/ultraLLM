...
class YellowEngine:
    ...
    def evaluate(self, query: Query, top_n: int = 3) -> EvidenceRecord:
        ...
        if not matches:
            return self._failed("No matching empirical data found.")

        best_index, best_raw_score = matches[0]
        score = bounded_score(best_raw_score)
        if score < self.minimum_score:
            return self._failed("No sufficiently relevant empirical data found.")
        ...
        return EvidenceRecord(
            ...
            trace=[f"BM25 matched {len(matches)} empirical records."],
            score=score,
